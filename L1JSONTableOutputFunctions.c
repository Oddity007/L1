...
static void call(uint64_t destination, uint64_t callee, uint64_t argumentCount, const uint64_t* arguments, void* userdata)
{
	...
	for (uint64_t i = 0; i < argumentCount; i++)
	{
		...
		if(i + 1 < argumentCount) fprintf(file, ", ");
	}
	fprintf(file, "]}, \n");
}
...
static void list(uint64_t destination, uint64_t elementCount, const uint64_t* elements, void* userdata)
{
	...
	for (uint64_t i = 0; i < elementCount; i++)
	{
		...
		if(i + 1 < elementCount) fprintf(file, ", ");
	}
	fprintf(file, "]}, \n");
}
...
static void loadUndefined(uint64_t destination, void* userdata)
{
	FILE* file = userdata;
...
const L1GenerateIROutputFunctions L1JSONTableOutputFunctions =
{
	loadInteger,
	call,
	closure,
